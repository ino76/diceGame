...
class Cup {
    ...
    return function(activeOrSelected, trueOrFalse) {
      ...
      for (let something of listOfSomething) {
        switch (activeOrSelected) {
          case "activated":
            if (something._activated === trueOrFalse) {
              list.push(something);
            }
            break;

          case "selected":
            if (something._selected === trueOrFalse) {
              list.push(something);
            }
            break;
        }
  ...
  addDiceListeners() {
    ...
    this.dice.forEach(d =>
      d.HTML.addEventListener("click", function(e) {
...
class Game {
  constructor() {
    ...
    // html bindings
    this.rideSound = document.getElementById("ride");
    ...
    // privitej hrace
    this.showLog("<span class='green'>Welcome to dice game. Good luck!</span>");
  ...
  evaluate(givenDice) {
    if (this.betweenRounds) {
      this.showLog(`<span class="red">Wait until next throw!</span>`);
      givenDice.forEach(d => d.prepare());
      return;
    } else {
      console.log("Nejsme mezi koly, nebo se neco podelalo ...");
    }
  ...
  pot() {
    ...
    if (points) {
      ...
      this.showLog(
        `You added <span class='white'>${points}</span> to round. <span class="grey">(temporary points)</span>`
      );

      if (this.cup.getActivated().length === 0) {
        this.showLog(`Dices re-activated.`);
        this.newThrow = true;
      }
      ...
      if (!this.betweenRounds) {
        this.showLog(
          `<span class="red">You must select some <span style="font-size:2.2em;">⚀</span> or <span style="font-size:2.2em;">⚄</span>, three of a kind or more ...</span>`
        );
      }
      this.cup.unselectAll();
    }
  ...
  safe() {
    ...
    if (points) {
      ...
      this.setPotPoints(0);
      this.showLog(
        `You saved <span class='gold'>${points}</span> to Total. <span class="grey">(permanently)</grey>`
      );
      this.newThrow = true;
  ...
  badLuck() {
    ...
    if (this.potPoints > 0) {
      this.showLog(
        `You have lost <span class='red'>${
          this.potPoints
        }</span> points. <span class="grey">(temporary points)</span>`
      );
    }
    this.setPotPoints(0);
  ...
  showValues() {
    let message = "New throw. ";

    if (this.player.rounds > 1) {
      if (this.player.lives > 1) {
        message += `<span class="grey">(<span class="green">${
          this.player.lives
        }</span> lives and <span class="green">${
          this.player.rounds
        }</span> rounds until end)</span>`;
      } else {
        message += `<span class="grey">(<span class="red">Last live</span> and <span class="green">${
          this.player.rounds
        }</span> rounds until end)</span>`;
      }
    } else {
      if (this.player.lives > 1) {
        message += `<span class="grey">(<span class="green">${
          this.player.lives
        }</span> lives and <span class="red">last round</span>)</span>`;
      } else {
        message += `<span class="grey">(<span class="red">Last live</span> and <span class="red">last round</span>)</span>`;
      }
    }
  ...
  endGame(message) {
    ...
    this.rideSound.play();
    this.cup.deactivateAll();
    if (Number(this.safePoints) > 100) {
      this.safePoints = 100;
    }
    this.scoreForm.value = this.safePoints;
    setTimeout(() => {
